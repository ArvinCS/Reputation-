...
public class ReputationDislike implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args){
		...
		if (cmd.getName().equals("reputationdislike")){
			if (args.length == 1){
				Player target = Bukkit.getServer().getPlayer(args[0]);
				if (target != null){
					if (player.getName().equals(target.getName())){
						player.closeInventory();
						player.sendMessage(prefix + ChatColor.WHITE + Message.read("message-cantdislike-yourself"));
					} else {
						...
						if (Rep.getPoint(player) > 0){
							...
							if (CooldownUtil.isExpired(player,cooldownname) == true){
								...
								String message1 = Message.read("message-disliked");
				...
				player.sendMessage(prefix + ChatColor.WHITE + Message.read("message-wrongcmd-repdislike"));
			}
		}
		return true;
	}
